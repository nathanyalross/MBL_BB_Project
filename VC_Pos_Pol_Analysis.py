...
def detect_events(data, time_axis, threshold_factor=3.0, min_amplitude=5.0,
                  min_rise_time=0.001, max_rise_time=0.010, min_decay_time=0.005):
    """
    Detect inhibitory postsynaptic currents (IPSCs) using median-based detection``
    Parameters:
    ...
    noise_trace = data - baseline
    noise_std = np.std(noise_trace)

    # Set detection threshold (positive for inhibitory events)
    ...
    min_distance_samples = int(0.010 * sampling_rate)  # 5ms (0.005) instead of 10ms (0.010)
...
def analyze_event(data, time_axis, peak_idx, baseline_value):
    ...
    # Baseline from 15 to 5 ms before peak
    # Baseline set as 15 miliseconds before peak to 5 miliseconds
    pre_start = max(0, peak_idx - int(0.015 / dt_seconds))
    pre_end = max(0, peak_idx - int(0.005 / dt_seconds))
    #Determines the mean value before event 
    event_baseline = np.mean(data[pre_start:pre_end]) if pre_end > pre_start else baseline_value

    # Define peak properties
    peak_time = time_axis[peak_idx]
    peak_amplitude = data[peak_idx] - event_baseline
    ...
    if decay_end_idx > decay_start_idx + 20:
        try:
            ...
            tempwav = data[decay_start_idx:decay_end_idx] - event_baseline
...
def plot_VC(file_path):
    ...
    with h5py.File(file_path, 'r') as f:
        ...
        for i, sweep_number in enumerate(available_sweeps):
            ...
            current_pA = apply_lowpass_filter(current_pA)  # Apply 5 Hz filter
        ...
        scale_bar_y = all_current.min() + 0.85 * current_range
...
#New Plot Event Detection function that will plot and save information for each sweep individually
    ...
    min_rise_time = 0.0005  # Minimum rise time in seconds (0.001 equals 1ms)
    max_rise_time = 0.0100  # Maximum rise time in seconds (0.1 equals 100ms)
    min_decay_time = 0.005  # Minimum decay time in seconds (5ms)
...
def plot_event_overlay_BL(file_path):
    ...
    max_decay_time = 0.20  # Maximum decay time in seconds (0.2 gets almost all the events)
